...
// ============ Model data ============
const MODELS = [
  // Interleaved across providers so the grid shows variety up front (flagship first).
...
const CatalogueScreen = () => {
  ...
  // Per-model sovereignty contribution — reacts to the real attributes so the
  // index moves as you filter (AI Act transparency, open licence, low carbon).
  const modelSov = (m) => {
    let s = 70;
    if (isAiActReady(m)) s += 14;                 // training-data transparency (AI Act)
    if (isOpenSourceLicense(m.license)) s += 10;  // open weights / OSI licence
    if (modelCarbonFR(m.id) <= 150) s += 6;       // low-carbon deployable
    return Math.min(100, s);
  };
  const sovScore = filtered.length ? Math.round(filtered.reduce((s, m) => s + modelSov(m), 0) / filtered.length) : 0;
